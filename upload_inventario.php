...
foreach($dadosPlanilha as $linha) {
  ...
  $sql_select = "SELECT * FROM inventario WHERE serialn = ?";
  $stmt_select = $conexao->prepare($sql_select);
  $stmt_select->bind_param('s', $serialn);
  $stmt_select->execute();
  ...
  if ($result_select->num_rows > 0) {
    ...
    echo "As informações que você está tentando adicionar, já existem no sistema para o número de produto $serialn.<br>";
    continue;
  }

  // Cria a consulta SQL para inserir os dados na tabela "inventario"
  $sql_insert = "INSERT INTO inventario (serialn, modelo, site, setor, quant, centrocusto) VALUES (?, ?, ?, ?, ?, ?)";
  ...
  $stmt_insert->bind_param('ssssss', $serialn, $modelo, $site, $setor, $quant, $centrocusto);
